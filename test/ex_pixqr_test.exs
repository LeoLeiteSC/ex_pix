defmodule ExPIXQRTest do
  use ExUnit.Case
  doctest ExPIXQR

  alias ExPIXQR.Data.Static
  ...
  describe "when parameters are correct" do
    test "should build a code if the parameters are correct", %{static_params: static_params} do
      assert {:ok, received_string} = ExPIXQR.generate_static_code("random_key", static_params)
  ...
  describe "when parameters are incorrect" do
    test "should return an error if a parameter is incorrect", %{static_params: static_params} do
      ...
      assert {:error, "The merchant_name is invalid"} == ExPIXQR.generate_static_code("random_key", new_params)
    end
  end
